...
func (suite *Snake) SetupSuite() {
	...
	suite.Require().Nil(err)
	err = suite.SendTransaction(key1)
	suite.Require().Nil(err)
	err = suite.SendTransaction(key2)
	suite.Require().Nil(err)
	err = suite.SendTransaction(key3)
	suite.Require().Nil(err)
	err = suite.SendTransaction(key4)
	suite.Require().Nil(err)
...
func (suite *Snake) SendTransaction(pk crypto.PrivateKey) error {
	node0 := &rpcx.NodeInfo{Addr: cfg.addrs[0]}
	client, err := rpcx.New(
		rpcx.WithNodesInfo(node0),
		rpcx.WithLogger(cfg.logger),
